...
namespace Core
{
    public class Magic
    {
        private readonly struct MagicEntry
        {
        ...
        private static readonly MagicEntry[] _rookMagic = BuildMagicRookTables(RookMagics);
        private static readonly MagicEntry[] _bishopMagic = BuildMagicBishopTables(BishopMagics);

        public static ulong GetRookMoves(Square square, ulong allPieces, ulong ownPieces) =>
            _rookMagic[(int)square].Lookup(allPieces, ownPieces);

        public static ulong GetBishopMoves(Square square, ulong allPieces, ulong ownPieces) =>
        ...
        private static MagicEntry FindRookMagic(Square square, int indexBits, Random rng)
        {
            int indexShift = 64 - indexBits;
            while (true)
            {
        ...
        private static MagicEntry FindBishopMagic(Square square, int indexBits, Random rng)
        {
            int indexShift = 64 - indexBits;
            ulong mask = BishopMask[(int)square];
            while (true)
